...
def Prepare_data(data_dir, label_file=None, id_name=None, label_name=None):
    ...
    if label_file is None or id_name is None or label_name is None:
        data_arr = []
        for file_name in tqdm(file_list, desc="读取数据文件"):
        ...
        for file_name in tqdm(file_list, desc="读取数据并匹配标签"):
            file_path = os.path.join(data_dir, file_name)
            this_np = pd.read_csv(file_path).to_numpy()
            data_arr.append(this_np)
        ...
        print(f"标签统计: 0 的数量 = {label_counts[0]}, 1 的数量 = {label_counts[1]}")
...
def evaluate_downstream(data_arr, label_arr, k=4, epochs=100, lr=0.02, seed=42):
    ...
    methods = [
        ...
        ('KNN-Impute', lambda:  Prepare_data('./data_imputed/knn/III', './AAAI_3_4_labels.csv', 'ICUSTAY_ID', tag)),
        ('MICE-Impute', lambda: Prepare_data('./data_imputed/mice/III', './AAAI_3_4_labels.csv', 'ICUSTAY_ID', tag)),
        ('TimeMixerPP-Impute', lambda:  Prepare_data('./data_imputed/timemixerpp/III', './AAAI_3_4_labels.csv', 'ICUSTAY_ID', tag)),
        ('SAITS-Impute', lambda: Prepare_data('./data_imputed/saits/III', './AAAI_3_4_labels.csv', 'ICUSTAY_ID', tag)),
        ('TEFN-Impute', lambda: Prepare_data('./data_imputed/tefn/III', './AAAI_3_4_labels.csv', 'ICUSTAY_ID', tag)),
        ('TimesNet-Impute', lambda:  Prepare_data('./data_imputed/timesnet/III', './AAAI_3_4_labels.csv', 'ICUSTAY_ID', tag)),
        
        ('TSDE-Impute', lambda: Prepare_data('./data_imputed/tsde/III', './AAAI_3_4_labels.csv', 'ICUSTAY_ID', tag)),
        ('Miracle-Impute', lambda:  Prepare_data('./data_imputed/miracle/III', './AAAI_3_4_labels.csv', 'ICUSTAY_ID', tag)),
    ...
    for method_name, data_func in tqdm(methods, desc="评估插补方法"):
        ...
        try:
            ...
            accs = train_and_evaluate(data_arr_method, label_arr_method, k=k, epochs=epochs, lr=lr, seed=seed)
